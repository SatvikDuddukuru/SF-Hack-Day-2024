...
def check_fonts(slide, fb):
    if slide.shapes.title is not None:
        ...
        if title_font not in ["Mecherle Sans Semibold", "Mecherle Sans Slab Regular"]:
            fb.add_feedback(f"• Change the font of the title '{slide.shapes.title}' from {title_font} to Mecherle Sans Semibold or Mecherle Sans Slab Regular")
            ...
            for paragraph in text_frame.paragraphs:
                if paragraph.level == 0:
                    if paragraph.font.name not in ["Mecherle Sans Semibold", "Mecherle Sans Slab Regular"]:
                        fb.add_feedback(f"• Change the font of '{paragraph.text}' from {paragraph.font.name} to Mecherle Sans Semibold or Mecherle Sans Slab Regular")
                else:
                    if paragraph.font.name not in ["Mecherle Sans Medium", "Mecherle Sans Regular", "Mecherle Sans Semibold"]:
                        fb.add_feedback(f"• Change the font of '{paragraph.text}' from {paragraph.font.name} to Mecherle Sans Medium, Regular, or Semibold")
...
def check_visibility(slide, fb):
    for shape in slide.shapes:
        if shape.has_text_frame:
            text_frame = shape.text_frame
            for paragraph in text_frame.paragraphs:
                if paragraph.font.size is not None and paragraph.font.size < 12:
                    fb.add_feedback(f"• Increase the font size from {paragraph.font.size} to at least 12")
                if len(paragraph.text.split(" ")) > 20:
                    fb.add_feedback(f"• Break this up into more easily digestible chunks")
